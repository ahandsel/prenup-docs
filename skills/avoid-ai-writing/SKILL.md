...
This repo's content is **bilingual** (`contents/en/` paired 1-to-1 with `contents/ja/`), and several sibling skills overlap with this one. Use the right tool for the job:

- **Language.** The vocabulary tables and phrase rules below are written for **English** prose. Japanese LLM output has different tells (see "Japanese AI-isms" below for the common ones); the full Japanese writing standard lives in the `japanese-tech-writing` skill (LLM っぽい空句の禁止) and `text-proofreading`. Do not apply the English word table to Japanese text.
- **Voice vs. mechanics.** This skill removes AI *voice and patterns*. The repo's mechanical writing rules - en-dash/em-dash to a plain hyphen, curly to straight quotes, sentence-case headings, the Oxford comma, no slash for "or"/"and", reference-style links - are enforced by the `general-en-polisher` and `link-polisher` skills. Where a rule here overlaps (em dashes, curly quotes, title-case headings), this skill flags the *pattern*; run `general-en-polisher` for the mechanical fix pass so the two do not fight.
- **Drafting from scratch.** To avoid AI-isms before they appear, use `thinking-first-draft` to capture the motive, audience, and one key point first - hollow, unaccountable prose is the root cause most of the rules below treat as symptoms.
- **Style guides win.** Defer to the guides in `docs/` and the terminology in `docs/glossary.yaml`. Where a repo guide conflicts with a default in this skill, the repo guide wins.
- **Domain.** This is prenuptial-agreement documentation, not crypto/web3 or travel content. The crypto-flavored examples below are illustrative of the *pattern*; for this repo, watch the legal/relationship boilerplate called out in "Domain boilerplate" instead.
...
#### Domain boilerplate - legal and relationship content

This repo is prenup documentation, so the crypto/web3 phrases above rarely appear. The equivalent filler here is reassurance-marketing and legal throat-clearing that wraps a template or guide without adding a clause, a number, or a step. Same rule as the table above: flag at 2+ uses of one phrase, or 3+ distinct phrases in one piece.

| Phrase | What to do |
|---|---|
| protect what matters most / safeguard your future | Name the specific asset, account, or obligation the clause covers |
| peace of mind | State what the agreement actually settles in advance (who pays X, who keeps Y) |
| start your marriage on the right foot / build a strong foundation | Cut. Say what the document does, not how the reader should feel about it |
| open and honest conversation(s) | Name the topic the couple needs to decide (debt disclosure, shared expenses) |
| every couple is unique / there's no one-size-fits-all | True but empty. Cut, or point to the specific clause the reader should customize |
| navigate the complexities of (marriage / divorce / Japanese law) | Name the actual provision or statute, or cut the framing |
| ensure a fair and equitable outcome | State the split, the formula, or the trigger condition the clause defines |

The skill flags the *writing pattern* only; it does not assess whether a clause is legally sound. Leave the legal substance to a qualified professional (see `AGENTS.md`).
...
- Note: the "note that" family ("it's worth noting that," "it is important to note that") recurs under Hollow intensifiers, Hedging, and Transition phrases too. Treat **Confidence calibration phrases** below as its canonical home and count it once when triaging by severity, not once per section.
...
- For this repo, the same move shows up as reassurance-brochure prose around legal templates: "Give yourselves the peace of mind every couple deserves" or "a prenup is the loving first step toward a strong, lasting marriage." Replace with plain description of what the document does: "This template covers separate property, shared living expenses, and divorce terms." See "Domain boilerplate" above for the recurring phrases.
...
## Japanese AI-isms (日本語)

The English word tables do not transfer to Japanese. When auditing `contents/ja/` (or any Japanese prose), use the tells below, and defer to the `japanese-tech-writing` skill for the full standard - these overlap with its "LLM っぽい空句の禁止" and "冗長の排除" rules. As with the English rules, these are signals, not proof; second-language and deadline-pressed writers produce them too.

### Empty closers and hollow predicates (空句)
- Sentences that end on a hollow, non-committal note: 「〜と言えるでしょう」「〜ではないでしょうか」「〜と言っても過言ではありません」「重要だと言えます」. They assert nothing. State the fact, or cut the sentence.
- The blog/SEO closer「いかがでしたか？」and the CTA filler「ぜひ〜してみてください」「〜してみてはいかがでしょうか」are near-definitive AI/SEO tells. Remove them.

### Collaborative and scaffolding openers
- 「〜していきましょう」「見ていきましょう」「考えていきましょう」 - the Japanese counterpart of "Let's explore." Start with the point.
- Over-scaffolding with「まず」「次に」「そして」「最後に」on every step when the structure is already obvious. Keep connectives that carry logic; cut the ones that only pad rhythm.

### Inflation and over-intensification
- Stacked intensifiers: 「非常に」「とても」「まさに」「極めて」repeated across a passage. Cut, or replace with a concrete number or example.
- Can-do inflation: 「〜することができます」where「〜できます」is enough. The long form is grammatical but reads as padded AI output at density.
- Significance inflation: 「〜は、〜において重要な役割を果たしています」「大きな転換点となりました」on routine matters. State what happened.

### Structural tells
- Strings of 体言止め (noun-ending fragments) used as pseudo-summary bullets - the Japanese form of the "bullet lists of bare noun phrases" rule above. Convert to full sentences or genuine list content.
- Formulaic「まとめ」 / 「おわりに」headings that only restate the intro. Cut, or make the heading specify what it concludes.
- Em-dash, 2 倍ダッシュ (「——」), and 中黒 (・) overuse in headings and prose - the repo and `japanese-tech-writing` both ban these in Japanese body text; this skill flags the *AI-pattern density*, but the mechanical fix belongs to the Japanese style rules.

### Notes
- Do not "humanize" by forcing casual register onto formal legal Japanese. A prenup template is 書き言葉 / です・ます or である調 by design; the goal is to remove hollow filler, not to loosen the register.
- For terminology, follow `docs/glossary.yaml` and the Japanese style guides rather than swapping in synonyms.
